...
namespace qstl {
	...
	{
		if (!s) s = "";
		return size() == strlen(s) && 0 == std::memcmp(c_str(), s, size());
	}
	...
	string_view temp = "hahaha";
	string_view fop = "hahaha";
	void test() {
		assert(temp == fop);
	//	temp.can_resize();
	}
	...
	{
		iterator ip(iat(ipo));
		size_type nti = std::distance(first, last), bti = 0;
...
#define __va_copy(x,y)
	...
	{
		return Utf8Bytes((wchar_t)size()) + size();
	}
	...
	size_t string::hash(const char* first, const char* last) noexcept // static
	{
		size_t h = 0;
		// This has the bits flowing into each other from both sides of the number
		for (; first < last; ++first)
			h = *first + ((h << 7) | (h >> (BitsInType(size_t(1)) - 7)));
		return h;
	}
