...
///
	// this is a const interface for the string class
	// it has all the basics for finding, searching, etc.
	class string_view : public cmemlink {
	public:
		using value_type = typename  cmemlink::value_type;
		using uvalue_type = typename  std::make_unsigned_t<value_type>;
		using pointer = typename  cmemlink::pointer;
		using const_pointer = typename  cmemlink::const_pointer;
		using reference = typename  cmemlink::reference;
		using const_reference = typename  cmemlink::const_reference;
		using value_type = typename  cmemlink::value_type;
		using iterator = typename  cmemlink::iterator;
		using const_iterator = typename  cmemlink::const_iterator;
		using reverse_iterator = typename  std::reverse_iterator<iterator>;
		using const_reverse_iterator = typename std::reverse_iterator<const_iterator>;
		using size_type = typename  cmemlink::size_type;
	...
	public:
		...
		string_view& assign(const_pointer s, size_type len) {
			if (s == nullptr|| len ==0) { s = ""; len = 0; }
			relink(s, len);
			return *this;
		}
		string_view& assign(const_pointer s) {
			return assign(s, std::strlen(s));
		}
		inline string_view&		assign(const_iterator i1, const_iterator i2) { return assign(i1, std::distance(i1, i2)); }
		inline string_view&		assign(const string_view& s) { return assign(s.begin(), s.end()); }
		inline string_view&		assign(const string_view& s, pos_type o, size_type n) { return assign(s.iat(o), s.iat(o + n)); }
		string_view() noexcept : cmemlink("",0) {}
		string_view(const string_view& s) noexcept: cmemlink(s.data(),s.size())  {}
		string_view(const_pointer s, size_type len) noexcept : cmemlink()  { assign(s, len); }
		string_view(const_pointer s) noexcept : string_view(s, std::strlen(s)) {}
		inline string_view(const cmemlink& s) noexcept  : string_view(s.data(), s.size())  {}

		/// Assigns itself a [o,o+n) substring of \p s.
		inline string_view(const string_view& s, pos_type o, size_type n) : cmemlink() { assign(s, o, n); }

		inline const_reference	operator[](pos_type i) const { return begin()[i]; }
		...
		static int compare(const_iterator first1, const_iterator last1, const_iterator first2, const_iterator last2) noexcept {
			...
			const int diff = int(len1 - len2);
			const int rvbylen = (diff < 0) - (diff < 0 ? -1 : 1);
			int rv = memcmp(first1, first2, std::min(len1, len2));
			return rv ? rv : rvbylen;
		}
		inline int			compare(const string_view& s) const { return compare(begin(), end(), s.begin(), s.end()); }
		inline int			compare(pos_type start, size_type len, const string_view& s) const { return compare(iat(start), iat(start + len), s.begin(), s.end()); }
		inline int			compare(pos_type s1, size_type l1, const string_view& s, pos_type s2, size_type l2) const { return compare(iat(s1), iat(s1 + l1), s.iat(s2), s.iat(s2 + l2)); }
		...
		pos_type find(const string_view& s, pos_type pos = 0) const noexcept
		{
			...
			for (; i < end() && (i = std::find(i, end(), endchar)) < end(); i += skip)
				if (std::memcmp(i - endi, s.begin(), s.size()) == 0)
		...
		inline pos_type		find(const_pointer p, pos_type pos, size_type count) const { string_view sp; sp.link(p, count); return find(sp, pos); }
		/// Returns the offset of the last occurence of character \p c before \p pos.
		pos_type rfind(value_type c, pos_type pos = 0) const noexcept
		{
		...
		pos_type rfind(const string_view& s, pos_type pos = 0) const noexcept
		{
		...
		inline pos_type		rfind(const_pointer p, pos_type pos, size_type count) const { string_view sp; sp.link(p, count); return rfind(sp, pos); }
		/// Returns the offset of the first occurence of one of characters in \p s of size \p n after \p pos.
		pos_type find_first_of(const string_view& s, pos_type pos = 0) const noexcept
		{
		...
		inline pos_type		find_first_of(value_type c, pos_type pos = 0) const { string_view sp(&c,1); return find_first_of(sp, pos); }
		inline pos_type		find_first_of(const_pointer p, pos_type pos, size_type count) const { string_view sp; sp.link(p, count); return find_first_of(sp, pos); }
		/// Returns the offset of the first occurence of one of characters not in \p s of size \p n after \p pos.
		pos_type find_first_not_of(const string_view& s, pos_type pos = 0) const noexcept
		{
			for (size_type i = std::min(size_type(pos), size()); i < size(); ++i)
				if (s.find(at(i)) == npos)
					return i;
		...
		inline pos_type		find_first_not_of(value_type c, pos_type pos = 0) const { string_view sp(&c, 1); return find_first_not_of(sp, pos); }
		inline pos_type		find_first_not_of(const_pointer p, pos_type pos, size_type count) const { string_view sp; sp.link(p, count); return find_first_not_of(sp, pos); }
		/// Returns the offset of the last occurence of one of characters in \p s of size \p n before \p pos.
		pos_type find_last_of(const string_view& s, pos_type pos) const noexcept
		{
		...
		inline pos_type		find_last_of(value_type c, pos_type pos = npos) const { string_view sp(&c, 1); return find_last_of(sp, pos); }
		inline pos_type		find_last_of(const_pointer p, pos_type pos, size_type count) const { string_view sp; sp.link(p, count); return find_last_of(sp, pos); }
		/// Returns the offset of the last occurence of one of characters not in \p s of size \p n before \p pos.
		pos_type find_last_not_of(const string_view& s, pos_type pos = 0) const noexcept
		{
			for (int i = std::min(pos, pos_type(size() - 1)); i >= 0; --i)
				if (s.find(at(i)) == npos)
					return i;
		...
		inline pos_type		find_last_not_of(value_type c, pos_type pos = npos) const { string_view sp(&c,1); return find_last_not_of(sp, pos); }
		inline pos_type		find_last_not_of(const_pointer p, pos_type pos, size_type count) const { string_view sp; sp.link(p, count); return find_last_not_of(sp, pos); }
		inline string_view		substr(pos_type o = 0, size_type n = npos) const { return string_view(*this, o, n); }
		inline void			swap(string_view& v) { cmemlink::swap(v); }
	};
	//----------------------------------------------------------------------
	// Operators needed to avoid comparing pointer to pointer

#define PTR_STRING_CMP(op) \
	inline bool operator##op (const char* s1, const string_view& s2) { return string_view::compare(s1,s1 + strlen(s1),s2.begin(),s2.end()) op 0; }
	PTR_STRING_CMP(== )
		PTR_STRING_CMP(!= )
		PTR_STRING_CMP(< )
		PTR_STRING_CMP(> )
...
#define PTR_STRING_CMP(op) \
	inline bool operator##op (const string_view& s1, const string_view& s2) { return s1.compare(s2) op 0; }
		PTR_STRING_CMP(== )
		PTR_STRING_CMP(!= )
		PTR_STRING_CMP(< )
		PTR_STRING_CMP(> )
...
inline void string::resize (size_type n, value_type c)
{
    ...
    resize (n);
    std::fill_n (iat(oldn), std::max(value_type(n-oldn), value_type(0)), c);
}

//----------------------------------------------------------------------
// Operators needed to avoid comparing pointer to pointer

#define PTR_STRING_CMP(op, impl) inline bool op (const char* s1, const string& s2) { return impl; }
