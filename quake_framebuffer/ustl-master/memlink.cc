...
void memlink::read (istream& is)
{
    ...
    is >> n;
    if (n > size())
		throw std::length_error ("memlink can not increase the size of the linked storage for reading");
    resize (n);
    is.read (data(), n);
	if(is.gcount() != n)
		throw std::length_error(" we read short data");

	size_t aligment = stream_align_of(n);
	while (aligment--) is.get();
}
