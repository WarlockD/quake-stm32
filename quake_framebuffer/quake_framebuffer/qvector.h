...
namespace quake {
	...
	namespace detail {
		...
		struct char_info {
			...
			constexpr static  size_type npos = size_type(-1); // std::numeric_limits<size_type>::max();
		};
...
#undef NUMBER_TO_STRING_CONVERTER
	...
	public:
		...
		inline size_t size() const { return ::strlen(_data); }
		inline size_t capacity() const { return _CAPACITY; }
		inline iterator		begin(void) { return _data.begin(); }
		inline iterator		end(void) { return begin() + size(); }
		inline reference	at(size_type i) { return _data[i]; }
		inline constexpr const_reference	at(size_type i) const { assert(i < size()); return _data[i]; }
		inline constexpr const_reference	operator[] (size_type i) const { assert(i < size()); return _data[i]; }
		...
		sstring() : _data{ '\0' } {}
		sstring(const char* s) { strcpy_s(data(), s); }
		template<size_t S>
		sstring(const sstring<S>& s) { strncpy_s(data(), s.data(), s.capacity()); }
		template<size_t S>
		sstring& operator=(const sstring<S>& s) { strncpy_s(data(), s.data(),s.capacity()); retirm *this; }

		bool operator==(const char* s) const { return strcmp(data(), s) == 0; }
		bool operator!=(const char* s) const { return strcmp(data(), s) != 0; }
		template<size_t S>
		bool operator==(const sstring<S>& s) const { return strcmp(data(), s.data()) == 0; }
		template<size_t S>
		bool operator!=(const sstring<S>&s) const { return strcmp(data(), s.data()) != 0; }
	private:
	...
	template<size_t S>
	inline bool operator==(const char* l, const sstring<S>& r)  { return strcmp(s, r.data()) == 0; }
	template<size_t S>
	inline bool operator!=(const char* l, const sstring<S>& r)  { return strcmp(s, r.data()) != 0; }
	...
	public:
		...
		template<typename ITR>
		void assign(const ITR& a, const ITR& b) {
			assert(a < b && std::distance(a, b) < capacity());
			if constexpr(std::is_copy_constructible<T>::value || !std::is_pointer<ITR>::value)
				std::uninitialized_copy(a, b, begin());
			else
				memcpy(_data, a, std::distance(a, b));
			resize_uninitialized(std::distance(a, b));
		}
		template<typename ITR>
		void assign(ITR&& a, ITR&& b) {
			assert(a < b && std::distance(a, b) < capacity());
			if constexpr(std::is_move_constructible<T>::value || !std::is_pointer<ITR>::value)
		...
		void append(ITR a, ITR b) {
			...
			if constexpr(std::is_move_constructible<T>::value || !std::is_pointer<ITR>::value)
				std::uninitialized_move(a, b, begin() + size());
			else
				memcpy(_data + size(), a, std::distance(a, b));
