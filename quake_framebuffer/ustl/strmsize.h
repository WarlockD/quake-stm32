...
inline streamsize stream_size_of (const T& v) {
	//using clean_t = std::remove_pointer<std::remove_cv<T>>;
    typedef typename tm::Select <std::is_arithmetic_v<T> || std::is_pointer_v<T>,
...
namespace ustl {		\
    template <> inline size_t stream_align_of<type> (const type##&) { return grain; } \
}
