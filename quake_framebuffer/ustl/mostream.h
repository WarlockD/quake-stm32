...
namespace priv2 {
	...
	template<typename T>
	inline auto serialize_imp(::ustl::ostream& os, const T& obj, write_stream)-> decltype(os << obj, void()) { os << obj; }
	template<typename T>
	inline auto serialize_imp(::ustl::ostream& os, const T& obj, write_stream_function)-> decltype(os.iread(obj), void()) { os.iwrite(obj); }
	template<typename T>
	inline auto serialize_imp(::ustl::ostream& os, const T& obj, write_object_itself) -> decltype(obj.text_write(os), void()) { obj.text_write(os); }
	template<typename T>
	inline auto serialize(::ustl::ostream& os, const  T& obj) { serialize_imp(os, obj, write_object_itself()); }
}

template <typename T>
inline ostream& operator<< (ostream& os, const T& v) {
	priv2::serialize(os, v);
	return os;
#if 0
	using clean_t = std::remove_cv_t<T>;
    typedef typename tm::Select <std::is_arithmetic_v<clean_t>,
