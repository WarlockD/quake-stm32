...
namespace ustl {
	class default_memory_resource_t : public memory_resource {
	protected:
		void* do_allocate(size_t bytes, size_t alignment) override final {
			return malloc(Align(bytes, alignment));
		}
		void* do_reallocate(void* p, size_t bytes, size_t alignment) override final {
			return realloc(p, Align(bytes, alignment));
		}
		void do_deallocate(void* p, size_t bytes, size_t alignment) override final {
			free(p);
		}
		bool do_is_equal(const memory_resource& other) const noexcept override final {
			return std::addressof(other) == this;
			//auto p = dynamic_cast<const default_memory_resource_t*>(&other);
			//return p != nullptr;
		}
	};
	default_memory_resource_t default_memory_resource;
	memory_resource* get_default_memory_resource() {
		return &default_memory_resource;
	}

	//
	memblock::memblock(memory_resource* r) noexcept		: memlink(), _capacity(0), _memory_resource(r ? r : &default_memory_resource) { }
	memblock::memblock(const void* p, size_type n) : memlink(), _capacity(0) { assign(p, n); }
	memblock::memblock(size_type n, memory_resource* r) : memlink(), _capacity(0), _memory_resource(r ? r : &default_memory_resource) { resize(n); }
	memblock::memblock(const cmemlink& b, memory_resource* r) : memlink(), _capacity(0), _memory_resource(r ? r : &default_memory_resource) { assign(b); }
	memblock::memblock(const memlink& b, memory_resource* r) : memlink(), _capacity(0), _memory_resource(r ? r : &default_memory_resource) { assign(b); }
	memblock::memblock(const memblock& b, memory_resource* r) : memlink(), _capacity(0), _memory_resource(r ? r : b.get_memory_resorce()) { assign(b); }
	...
	void memblock::shrink_to_fit(void)
	{
		if (is_linked())
			return;
		pointer newBlock = (pointer)_memory_resource->reallocate(begin(), size());
		if (!newBlock && size())
