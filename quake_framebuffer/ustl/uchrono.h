...
struct duration_values {
    ...
    static constexpr Rep	min (void)	{ return std::numeric_limits<Rep>::lowest(); }
    static constexpr Rep	max (void)	{ return std::numeric_limits<Rep>::max(); }
};

template <typename Rep>
struct treat_as_floating_point : public std::is_floating_point<Rep> {};
...
struct duration {
    ...
    template <typename Rep2, typename = typename
		std::enable_if<std::is_convertible<Rep2, rep>::value
			&& (treat_as_floating_point<rep>::value
			    || std::numeric_limits<Rep2>::is_integer)>::type>
    ...
    template <typename Rep2 = rep> typename std::enable_if<std::numeric_limits<Rep2>::is_integer,
    ...
    inline constexpr typename std::enable_if<
		ratio_greater<ratio_divide<period,Period2>,ratio<1>>::value
		    && std::is_convertible<duration,duration<Rep2,Period2>>::value,
...
struct hrtime_t : public timespec {
    ...
    inline explicit constexpr	hrtime_t (float v)		: timespec{time_t(v),long((v-time_t(v))*nano::den)}{}
    inline explicit constexpr	hrtime_t (double v)		: timespec{time_t(v),long((v-time_t(v))*nano::den)}{}
...
template <> struct duration_values<hrtime_t> {
	...
	{ return hrtime_t (std::numeric_limits<decltype(hrtime_t::tv_sec)>::lowest(), nano::den-1); }
    static constexpr hrtime_t max (void)
	{ return hrtime_t (std::numeric_limits<decltype(hrtime_t::tv_sec)>::max(), nano::den-1); }
};

// timespec has a very high resolution, so is convertible as float
template <> struct treat_as_floating_point<hrtime_t> : public std::true_type {};
    ...
    static time_point now (void) noexcept {
		return time_point(
			duration_cast<duration>(
				high_resolution_clock::duration(rep_now (CLOCK_MONOTONIC)))
		);
    }
