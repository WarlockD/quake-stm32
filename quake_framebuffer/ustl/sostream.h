...
{
    char fmt [16];
    fmtstring (fmt, printf_typestring(v), std::numeric_limits<T>::is_integer);
    format (fmt, v);
}
...
template <typename T> struct integral_text_object_writer {
    inline void operator()(ostringstream& os, const T& v) const { os.iwrite (std::forward<T>(v)); }
};
namespace priv {

	struct write_stream {};
	struct write_stream_function : write_stream {};
	struct write_object_itself : write_stream_function {};
	template<typename T>
	inline auto serialize_imp(::ustl::ostringstream& os,const T& obj, write_stream)-> decltype(os << obj, void()) { os << obj; }
	template<typename T>
	inline auto serialize_imp(::ustl::ostringstream& os, const T& obj, write_stream_function)-> decltype(os.iread(obj), void()) { os.iwrite(obj); }
	template<typename T>
	inline auto serialize_imp(::ustl::ostringstream& os, const T& obj, write_object_itself) -> decltype(obj.text_write(os), void()) { obj.text_write(os); }
	template<typename T>
	inline auto serialize(::ustl::ostringstream& os, const  T& obj) { serialize_imp(os, obj, write_object_itself()); }
}
...
inline ostringstream& operator<< (ostringstream& os, const T& v) {
	...
	using object_writer_t = std::conditional_t<
		std::is_
		std::is_arithmetic_v<T> ||
		std::is_pointer_v<T> ||
		std::is_convertible_v<T, long>,
		integral_text_object_writer<T>, object_text_writer<T>>;
    object_writer_t()(os, std::forward<T>(v));
    return os;
#endif
}
...
namespace {
	...
	static constexpr const Sflush flush;
