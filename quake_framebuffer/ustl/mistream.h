...
namespace ustl {
	...
	namespace priv3 {
		struct read_from_stream {};
		struct read_from_stream_function : read_from_stream {};
		struct read_object_itself : read_from_stream_function {};
		template<typename T>
		inline auto serialize_imp(::ustl::istream& is, T& obj, read_from_stream)-> decltype(is >> obj, void()) { is >> obj; }
		template<typename T>
		inline auto serialize_imp(::ustl::istream& is, T& obj, read_from_stream_function)-> decltype(is.iread(obj), void()) { is.iread(obj); }
		template<typename T>
		inline auto serialize_imp(::ustl::istream& is, T& obj, read_object_itself) -> decltype(obj.read(is), void()) { obj.read(is); }
		template<typename T>
		inline auto serialize(::ustl::istream& is, T& obj)  { serialize_imp(is, obj, read_object_itself()); }
	}

	template <typename T>
	static inline istream& operator>> (istream& is, T& v) {
		priv3::serialize(is, v);
#if 0
		typedef typename tm::Select <std::is_integral<T>::value,
