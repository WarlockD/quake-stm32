...
template <> struct name<type> : public true_type {}
	...
	struct ___is_destructible {
		template <typename T, typename = decltype(declval<T&>().~T())> static true_type test(int);
		template <typename T> static false_type test(...);
	};
	template <typename T> struct is_destructible : public decltype(___is_destructible::test<T>(0)) {};

	struct ___is_nothrow_destructible {
		template <typename T> static integral_constant<bool, noexcept(declval<T&>().~T())> test(int);
		template<typename> static false_type test(...);
	};
	template<typename T>
	struct is_nothrow_destructible : public decltype(___is_nothrow_destructible::test<T>(0)) {};
	...
	template <typename T, typename U> struct is_assignable : public decltype(___is_assignable::test<T, U>(0)) {};
#endif
