...
template<typename T> static inline T __atomic_fetch_##NAME(T* var, const T v,memory_order order) { T tmp = *var; *var OP= tmp OP v; return tmp; } \
template<typename T> static inline T __atomic_##NAME##_fetch(T* var, const T v,memory_order order) { T tmp = *var OP v;  return (*var = tmp); } 
	...
	ATOMIC_FETCH_OP(add, +)
		...
		template<typename T> 
	static inline bool __atomic_store_n(T* var, const T v, memory_order order) {
		*var = v;
		return *var == v;
	}
	template<typename T>
	static inline T __atomic_load_n(T* var,  memory_order order) {
		T tmp = *var;
		return tmp;
	}
	static inline void __atomic_clear(bool* var, memory_order order) {
		*var = false;
	}
	static inline bool __atomic_test_and_set(bool* var, memory_order order) {
		bool v = *var;
		*var = true;
		return v;
	}
...
#define __atomic_exchange_n(A,B,C) 0

#define __atomic_is_lock_free(A,B) 0
#define __atomic_compare_exchange_n(A,B,C,D,E,F) 0

#define __atomic_thread_fence(A) 0
#define __atomic_signal_fence(A) 0
#endif
//}}}-------------------------------------------------------------------
//{{{ atomic
