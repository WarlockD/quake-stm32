...
/// of the required dereference operator.
	...
	inline typename __make_unique<T>::__single_object
		make_unique(Args&&... args) { return unique_ptr<T>(new T(std::forward<Args>(args)...)); }

	template <typename T>
	inline typename __make_unique<T>::__array
		make_unique(size_t n) { return unique_ptr<T>(new std::remove_extent_t<T>[n]()); }
...
/// Calls delete in the destructor; assignment shares ownership.
	...
	class scope_exit {
	public:
		inline explicit	scope_exit(F&& f) noexcept : _f(std::move(f)), _enabled(true) {}
		inline		scope_exit(scope_exit&& f) noexcept : _f(std::move(f._f)), _enabled(f._enabled) { f.release(); }
	...
	public:
		...
		inline		unique_resource(unique_resource&& r) noexcept
			: _resource(move(r._resource)), _deleter(move(r._deleter)), _enabled(r._enabled) {
			r.release();
		}
		...
		inline std::add_lvalue_reference_t<std::remove_pointer_t<R>>
